...
static void record_transfer(size_t len, int data_error) {
  if (add_u64(&report.transfers, 1) < 0 ||
      add_u64(&report.bytes, (uint64_t)len) < 0 ||
...
static int report_open(const test_options_t *opt) {
  ...
  if (opt->report_name) {
    ...
    snprintf(report.path, sizeof(report.path),
  ...
  if (opt->qualification) {
    if (snprintf(report.samples_path, sizeof(report.samples_path), "%s.csv",
                 report.path) >= (int)sizeof(report.samples_path) ||
...
static int report_close(void) {
  ...
  report_printf("\n## 汇总\n\n- 通过: %" PRIu64 "\n- 失败: %" PRIu64
                "\n- 跳过: %" PRIu64 "\n- transfer 次数: %" PRIu64
                "\n- 检查字节: %" PRIu64 "\n- 数据错误: %" PRIu64 "\n",
                report.pass, report.fail, report.skip, report.transfers,
                report.bytes, report.errors);
...
static void print_driver_parameters(unsigned int bus) {
  ...
  for (i = 0; i < sizeof(items) / sizeof(items[0]); i++) {
    ...
    if (snprintf(path, sizeof(path), "/sys/module/soc_spi/parameters/%s",
                 items[i]) >= (int)sizeof(path) ||
        read_text_value(path, value, sizeof(value)) < 0)
      snprintf(value, sizeof(value), "(不可读取)");
    written = snprintf(detail + used, sizeof(detail) - used, "%s%s=%s",
                       used ? " " : "", items[i], value);
...
static int configure_spi(spi_context_t *ctx, unsigned int mode,
  ...
  if (spi_set_mode(ctx->fd, (int)mode) < 0 ||
      spi_set_speed(ctx->fd, (int)speed) < 0 || spi_set_bits(ctx->fd, 8) < 0 ||
...
static int raw_transfer(int fd, void *tx, void *rx, size_t len,
                        unsigned int speed, unsigned int cs_change) {
...
static void fill_pattern(unsigned char *buffer, size_t len, unsigned int kind,
                         uint32_t seed) {
  size_t i;
  uint32_t state = seed;
...
static int checked_transfer(spi_context_t *ctx, unsigned char *tx,
                            unsigned char *rx, size_t len, unsigned int kind,
                            uint32_t seed, size_t *bad_offset) {
  int ret;
...
static int run_checked_once(spi_context_t *ctx, int internal_loop, size_t len,
                            uint32_t seed, size_t *bad_offset) {
  unsigned char *tx;
  unsigned char *rx;
  int ret;

  if (configure_spi(ctx, SPI_MODE_0 | (internal_loop ? SPI_LOOP : 0),
                    TEST_SCLK_HZ) < 0)
    return -EINVAL;
  tx = malloc(len);
  rx = malloc(len);
...
static scan_result_t run_frequency_scan(spi_context_t *ctx,
  ...
  for (i = 0; i < sizeof(scan_speeds) / sizeof(scan_speeds[0]) &&
    ...
    for (pattern = 0; pattern < opt->scan_loops; pattern++) {
      fill_pattern(tx, opt->max_transfer, pattern % 4U,
                   UINT32_C(0x13579bdf) + pattern);
    ...
    if (ret) {
      snprintf(detail, sizeof(detail),
               "轮数=%u 返回=%d 首个错误偏移=%zu 完成=%" PRIu64
               "B 有效带宽=%.2fMbit/s",
               pattern, ret, bad, bytes, mbps);
      case_result("时钟与带宽", name, -1, detail);
    } else {
      snprintf(detail, sizeof(detail),
               "图样轮数=%u 完成=%" PRIu64 "B 有效带宽=%.2fMbit/s",
               opt->scan_loops, bytes, mbps);
      case_result("时钟与带宽", name, 1, detail);
...
static void run_instrument_capture(spi_context_t *ctx,
                                   const test_options_t *opt,
                                   unsigned int speed, const char *cs_label,
                                   const char *kind) {
  ...
  if (ret >= 0) {
    ...
    if (opt->cs_to_clk_ns_valid) {
      snprintf(detail, sizeof(detail), "实测=%uns CS=%s", opt->cs_to_clk_ns,
               cs_label);
      case_result("CS 到首个 SCLK", kind, 1, detail);
    } else if (prompt_uint_measurement("输入 CS 下降沿到首个 SCLK 的间隔(ns)",
                                       &measured) > 0) {
...
static int run_preflight(spi_context_t *ctx, unsigned int bus) {
  ...
  for (group = 0; group < sizeof(groups) / sizeof(groups[0]); group++) {
    for (index = 0; index < group_counts[group] && !stop_requested; index++) {
      ...
      if (ret) {
        snprintf(detail, sizeof(detail), "内部 LOOP 返回=%d 错误偏移=%zu", ret,
                 bad);
      ...
      if (ret) {
        snprintf(detail, sizeof(detail),
                 "外部全双工返回=%d 错误偏移=%zu 完成=%" PRIu64 "B", ret, bad,
                 bytes);
        case_result("预检", name, -1, detail);
        return -1;
      }
...
static int write_sample_csv(unsigned int bus, const char *phase, size_t len,
  ...
  if (fprintf(report.samples_fp, "%u,%s,%zu,%u,%.3f,%" PRIu64 ",%.6f\n", bus,
              phase, len, sample, seconds * 1000.0, bytes, mbps) < 0)
    return -1;
...
static int run_sample_set(spi_context_t *ctx, unsigned int bus,
                          const char *phase, size_t len, unsigned int count,
                          double minimum_seconds, uint32_t *seed) {
  spi_stats_t stats;
  ...
  for (sample = 0; sample < count && !stop_requested; sample++) {
    ...
    double mbps;
    size_t bad;
    int ret = run_duplex_window(ctx, len, minimum_seconds, seed, &bytes,
                                &seconds, &bad);
    ...
    if (ret || seconds <= 0.0 || add_u64(&total_bytes, bytes) < 0 ||
      ...
      snprintf(detail, sizeof(detail),
               "样本=%u/%u 返回=%d 错误偏移=%zu 完成=%" PRIu64 "B", sample + 1,
               count, ret, bad, bytes);
      case_result("资格测试", name, -1, detail);
      return -1;
    }
    mbps = spi_stats_mean(&stats);
    if (write_sample_csv(bus, phase, len, sample + 1, seconds, bytes,
                         (bytes * 8.0 / 1000000.0) / seconds) < 0) {
...
static int run_stress(spi_context_t *ctx, unsigned int bus, uint32_t *seed) {
  ...
  do {
    ...
    if (ret || add_u64(&bytes, STRESS_LENGTH) < 0 ||
  ...
  if (ret || stop_requested) {
    snprintf(detail, sizeof(detail),
             "返回=%d 错误偏移=%zu 完成=%" PRIu64 "B transfer=%" PRIu64
             " 耗时=%.3fs",
             ret, bad, bytes, transfers, seconds);
    case_result("压力测试", "外部全双工随机数据", -1, detail);
    return -1;
  }
  snprintf(detail, sizeof(detail),
           "完成=%" PRIu64 "B transfer=%" PRIu64
           " 耗时=%.3fs 有效带宽=%.3fMbit/s 数据错误=0",
...
static int run_qualification_bus(const qualification_profile_t *profile,
                                 const test_options_t *opt,
                                 size_t spidev_bufsiz) {
  spi_context_t ctx;
  scan_result_t scan;
  ...
  report_printf("\n# SPI%u 资格测试\n\n- 引脚: CLK=%s MOSI=%s MISO=%s CS=%s\n",
                profile->bus, profile->clk, profile->mosi, profile->miso,
                profile->cs);
  ...
  if (scan.highest_speed != TEST_SCLK_HZ) {
    snprintf(detail, sizeof(detail), "最高无错档=%uHz，资格测试要求 60000000Hz",
             scan.highest_speed);
  ...
  if (!failed) {
    ...
    for (i = 0; i < sizeof(primary_lengths) / sizeof(primary_lengths[0]); i++) {
      if (run_sample_set(&ctx, profile->bus, "primary", primary_lengths[i],
                         1000U, 2.5, &seed) < 0) {
        failed = 1;
        break;
      }
  ...
  if (!failed) {
    report_printf("\n## SPI%u 边界带宽统计\n\n", profile->bus);
    for (i = 0; i < sizeof(boundary_lengths) / sizeof(boundary_lengths[0]);
         i++) {
      if (run_sample_set(&ctx, profile->bus, "boundary", boundary_lengths[i],
                         60U, 5.0, &seed) < 0) {
        failed = 1;
        break;
      }
